...
public class TripDateView extends LinearLayout {
    ...
    private void initDateAdapter(Context context) {
        ...
        tripDateAdapter.setOnItemClickListener((adapter, view, position) -> {
            if (itemClickListener != null) {
                ...
                LogUtil.E("tripDateAdapter---"+tripDateAdapter.isHaveSameDay(tripDateAdapter.getItem(position)));

                if (tripDateAdapter.isHaveSameDay(tripDateAdapter.getItem(position))) {  //有预约时才可以点击
                    itemClickListener.OnItemClick(getSameData(tripDateAdapter.getItem(position)), position);
                }
    ...
    public void setCompareData(List<MouthAppointmentModel> datas) {
        ...
        List<TripDateModel> tripDateModelList = new ArrayList<>();
...
//            Date end = new Date(data.getEndDatetime());
            ...
            TripDateModel model = new TripDateModel();
            model.setDate(DateUtil.parse(data.getDate(), DateUtil.DATE_YMD));
            model.setColor(data.getColor());
            tripDateModelList.add(model);

//            LogUtil.E("data.getColor()="+data.getColor());
//
//            for (Date date : DateUtil.getDatesBetweenTwoDate(start, end)) {
//
//            }

        }

        LogUtil.E("tripDateModelList()="+tripDateModelList.size());

        tripDateAdapter.setCompareData(tripDateModelList);
    }
    ...
    private List<MouthAppointmentModel> getSameData(Date date) {
        ...
        for (MouthAppointmentModel mouthAppointmentModel : mCopareDatas) {
            ...
            if (mouthAppointmentModel == null || TextUtils.isEmpty(mouthAppointmentModel.getDate())) {
                continue;
            }
            ...
            String date2 = f.format(DateUtil.parse(mouthAppointmentModel.getDate(), DateUtil.DATE_YMD));

            LogUtil.E("date1="+date1);
...
//            }
    ...
    public interface OnClickListener {
        void OnItemClick(List<MouthAppointmentModel> dateModels, int position);
