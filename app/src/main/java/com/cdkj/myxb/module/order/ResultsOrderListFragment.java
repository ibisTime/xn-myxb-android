...
import com.cdkj.myxb.adapters.ResultsOrderListAdapter;
...
public class ResultsOrderListFragment extends AbsRefreshListFragment {
     ...
     * @return
     */
    public static ResultsOrderListFragment getInstance(String state, String type, boolean isFirstRequest) {
        ResultsOrderListFragment fragment = new ResultsOrderListFragment();
    ...
    public RecyclerView.Adapter getListAdapter(List listData) {
        ResultsOrderListAdapter appointmentListAdapter = new ResultsOrderListAdapter(listData);
        appointmentListAdapter.setOnItemChildClickListener((adapter, view, position) -> {
            ...
            switch (view.getId()) {
                case R.id.tv_to_comment: //评价
                    AppointmentCommentActivity.open(mActivity, listModel.getCode(), mType, listModel.getOwner());
                    break;
        ...
        appointmentListAdapter.setOnItemClickListener((adapter, view, position) -> { //评价
            ...
            if (TextUtils.equals(listModel.getType(), UserHelper.S) && TextUtils.equals(listModel.getStatus(), OrderHelper.APPOINTMENT_6)) {   //如果是 专家并且是已完成状态
                ExpertsOrderDetailActivity.open(mActivity, listModel.getCode(), mType);
                return;
            }
            ResultOrderDetailActivity.open(mActivity, listModel.getCode(), mType);
        });
    ...
    public void getListRequest(int pageIndex, int limit, boolean isShowDialog) {
        ...
        call.enqueue(new BaseResponseModelCallBack<ResponseInListModel<AppointmentListModel>>(mActivity) {
            ...
            protected void onSuccess(ResponseInListModel<AppointmentListModel> data, String SucMessage) {
                mRefreshHelper.setData(data.getList(), "暂无成果订单", 0);
            }
